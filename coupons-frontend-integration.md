...
## Order/payment payload — not covered by the backend spec above

`POST /orders` and `POST /payments/orders` (this frontend's `createOrder`
and `createRazorpayOrder` in `src/lib/catalogApi.js`) aren't part of the
spec this doc was generated from, and no coupon/discount field is
documented for either. To make sure the amount actually charged matches
what the shopper saw at checkout, the frontend now sends
`coupon_code` and `discount_amount` on the order payload whenever a coupon
is applied, alongside the existing shipping/items fields — **this is a
frontend-side assumption, not a confirmed contract**. Backend team: please
confirm these are the field names you expect (or tell us the right ones),
and that `discount_amount` is applied server-side rather than trusted
